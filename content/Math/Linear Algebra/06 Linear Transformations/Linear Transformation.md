...
Let $V,W$ be [[Vector Space|Vector Spaces]] over the same [[Field]] $K$. A function $f: V \rightarrow W$ is a linear transformation if for any 2 vectors $\mathbf{v,u} \in V$ and any scalar $c \in K$:
1. [[Vector Addition]] 
$$
f(\mathbf{u+v}) = f(\mathbf{u}) + f(\mathbf{v})
$$
